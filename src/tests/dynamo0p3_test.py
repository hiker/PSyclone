...
def test_field_qr():
    ...
    output = (
        ...
        "      ALLOCATE (basis_w1(dim_w1, ndf_w1, nqp_xy, nqp_z))\n"
        "      !\n"
        "      ! Initialise sizes and allocate any basis arrays for w2\n"
        "      !\n"
        "      ndf_w2 = f2_proxy%vspace%get_ndf()\n"
        ...
        "      ALLOCATE (diff_basis_w2(diff_dim_w2, ndf_w2, nqp_xy, nqp_z))\n"
        "      !\n"
        "      ! Initialise sizes and allocate any basis arrays for w3\n"
        "      !\n"
        "      ndf_w3 = m2_proxy%vspace%get_ndf()\n"
        ...
        "      ALLOCATE (basis_w3(dim_w3, ndf_w3, nqp_xy, nqp_z))\n"
        "      diff_dim_w3 = m2_proxy%vspace%get_dim_space_diff()\n"
        "      ALLOCATE (diff_basis_w3(diff_dim_w3, ndf_w3, nqp_xy, nqp_z))\n"
        "      !\n"
        "      ! Compute basis arrays\n"
        "      !\n"
        "      CALL f1_proxy%vspace%compute_basis_function_3d_xyoz(basis_w1, "
        "ndf_w1, nqp_xy, nqp_z, xqp_xy, xqp_z)\n"
        "      CALL f2_proxy%vspace%compute_diff_basis_function_3d_xyoz("
        "diff_basis_w2, ndf_w2, nqp_xy, nqp_z, xqp_xy, xqp_z)\n"
        "      CALL m2_proxy%vspace%compute_basis_function_3d_xyoz(basis_w3, "
        "ndf_w3, nqp_xy, nqp_z, xqp_xy, xqp_z)\n"
        "      CALL m2_proxy%vspace%compute_diff_basis_function_3d_xyoz("
        "diff_basis_w3, ndf_w3, nqp_xy, nqp_z, xqp_xy, xqp_z)\n"
        "      !\n"
        "      ! Call kernels and communication routines\n"
        "      !\n"
        "      IF (f2_proxy%is_dirty(depth=1)) THEN\n"
        ...
        "map_w3, basis_w3, diff_basis_w3, nqp_xy, nqp_z, wqp_xy, wqp_z)\n"
        "      END DO \n"
        "      !\n"
        "      ! Set halos dirty for fields modified in the above loop\n"
        "      !\n"
...
def test_operator_different_spaces():
    ...
    output = (
        ...
        "      ! Compute basis arrays\n"
        "      !\n"
        "      CALL mapping_proxy%fs_to%compute_basis_function_3d_xyoz("
        "basis_w3, ndf_w3, nqp_xy, nqp_z, xqp_xy, xqp_z)\n"
        "      CALL mapping_proxy%fs_from%compute_diff_basis_function_3d_xyoz("
        "diff_basis_w2, ndf_w2, nqp_xy, nqp_z, xqp_xy, xqp_z)\n"
        "      CALL chi_proxy(1)%vspace%compute_diff_basis_function_3d_xyoz("
        "diff_basis_w0, ndf_w0, nqp_xy, nqp_z, xqp_xy, xqp_z)\n"
        "      !\n"
        "      ! Call kernels and communication routines\n"
        "      !\n"
        ...
        "map_w0, diff_basis_w0, nqp_xy, nqp_z, wqp_xy, wqp_z)\n"
        "      END DO \n"
        "      !\n"
        "      ! Deallocate basis arrays\n"
        "      !\n"
...
def test_operator_orientation_different_space():
    ...
    assert (
        "CALL my_mapping_proxy%fs_to%compute_basis_function_3d_xyoz(basis_w1, "
        "ndf_w1, nqp_xy, nqp_z, xqp_xy, xqp_z)" in gen_str)
    assert (
...
def test_operator_any_space_different_space_2():
    ...
    assert str(generated_code).find(
        "CALL b_proxy%fs_to%compute_basis_function_3d_xyoz") != -1
    assert str(generated_code).find(
        "CALL d_proxy%fs_from%compute_basis_function_3d_xyoz") != -1
    assert str(generated_code).find(
        "CALL d_proxy%fs_from%compute_diff_basis_function_3d_xyoz") != -1
    assert str(generated_code).find(
...
def test_basis():
    ...
    output = (
        ...
        "dimension(3,ndf_w2h,nqp_xy,nqp_z) :: basis_w2h\n"
        "      INTEGER, intent(in) :: ndf_w2v\n"
        "      INTEGER, intent(in), dimension(ndf_w2v) :: map_w2v\n"
        "      REAL(KIND=r_def), intent(in), "
        "dimension(3,ndf_w2v,nqp_xy,nqp_z) :: basis_w2v\n"
...
def test_diff_basis():
    ...
    output = (
        ...
        "      INTEGER, intent(in) :: ndf_w2h\n"
        "      REAL(KIND=r_def), intent(in), "
        "dimension(1,ndf_w2h,nqp_xy,nqp_z) :: diff_basis_w2h\n"
        "      INTEGER, intent(in) :: ndf_w2v\n"
        "      INTEGER, intent(in), dimension(ndf_w2v) :: map_w2v\n"
        "      REAL(KIND=r_def), intent(in), "
        "dimension(1,ndf_w2v,nqp_xy,nqp_z) :: diff_basis_w2v\n"
        "      INTEGER, intent(in) :: nqp_xy, nqp_z\n"
        "      REAL(KIND=r_def), intent(in), dimension(nqp_xy) :: wqp_xy\n"
        "      REAL(KIND=r_def), intent(in), dimension(nqp_z) :: wqp_z\n"
        "    END SUBROUTINE dummy_code\n"
