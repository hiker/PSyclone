...
from parse import Descriptor, KernelType
import expression as expr
import fparser
import os

valid_function_space_names = ["w0","w1","w2","w3","any_space_1","any_space_2","any_space_3","any_space_4","any_space_5","any_space_6","any_space_7","any_space_8","any_space_9"]
        ...
        for idx,arg in enumerate(func_type.args):
            if idx==0:
                if arg.name not in valid_function_space_names:
                    raise ParseError("Each meta_func 1st argument must be one of {0} for the dynamo0.3 api, but found '{1}' in '{2}".format(valid_function_space_names,arg.name,func_type))
    ...
    def __init__(self, arg_type):
        ...
        if self._type == "gh_field":
            ...
            if arg_type.args[2].name not in valid_function_space_names:
                raise ParseError("Each meta_arg 3rd argument must be one of {0} for the dynamo0.3 api, but found '{1}' in '{2}".format(valid_function_space_names,arg_type.args[2].name,arg_type))
            ...
            if arg_type.args[2].name not in valid_function_space_names:
                raise ParseError("Each meta_arg 3rd argument must be one of {0} in the dynamo0.3 api, but found '{1}' in '{2}".format(valid_function_space_names,arg_type.args[2].name,arg_type))
            self._function_space1 = arg_type.args[2].name
            if arg_type.args[3].name not in valid_function_space_names:
                raise ParseError("Each meta_arg 4th argument must be one of {0} in the dynamo0.3 api, but found '{1}' in '{2}".format(valid_function_space_names,arg_type.args[2].name,arg_type))
