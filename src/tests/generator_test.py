...
class TestGenerate:
    ...
    def test_non_existant_script_filename(self):
        ...
        root_path = os.path.dirname(os.path.abspath(__file__))
        with pytest.raises(IOError):
            alg, psy = generate(os.path.join(root_path, "test_files",
                                             "dynamo0p3",
                                             "1_single_invoke.f90"),
                                api="dynamo0.3",
    ...
    def test_script_file_too_short(self):
        ...
        with pytest.raises(GenerationError):
            alg, psy = generate(os.path.join(root_path, "test_files",
                                             ...
                                             "1_single_invoke.f90"),
                                api="dynamo0.3",
                                script_name=os.path.join(root_path,
                                                         "test_files",
                                                         "dynamo0p3", "xyz"))
        ...
        with pytest.raises(GenerationError):
            alg, psy = generate(os.path.join(root_path, "test_files",
                                ...
                                script_name=os.path.join(
                                    root_path, "test_files", "dynamo0p3",
                                    "1_single_invoke.f90"))
    ...
    def test_script_no_trans(self):
        ...
        with pytest.raises(GenerationError):
            alg, psy = generate(os.path.join(root_path, "test_files",
                                             ...
                                             "1_single_invoke.f90"),
                                api="dynamo0.3",
                                script_name=os.path.join("test_files",
                                                         "dynamo0p3",
                                                         "no_trans.py"))
    ...
    def test_script_null_trans(self):
        ...
        import pytest
        root_path = os.path.dirname(os.path.abspath(__file__))
        alg1, psy1 = generate(os.path.join(root_path, "test_files",
                                           "dynamo0p3",
                                           "1_single_invoke.f90"),
                              api="dynamo0.3")
        alg2, psy2 = generate(os.path.join(root_path, "test_files",
                                           "dynamo0p3",
                                           "1_single_invoke.f90"),
                              api="dynamo0.3",
